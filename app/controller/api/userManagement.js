...
class UserManagementController extends Controller {
  async getList() {
    ...
    */
    ...
    const query = {};
    if (request.userName) { query.userName = request.userName; }
  ...
  async doEdit() {
    ...
    */
    ...
    if (request.id) {
      // 请求体包含id字段，说明是编辑
      if (await ctx.helper.isRepeat(Collection, { userName: request.userName, id: { $ne: request.id } }) > 0) {
        ctx.body = {
          code: 400,
          msg: '用户名已存在！',
        };
      ...
      if (await ctx.helper.isRepeat(Collection, { userName: request.userName }) > 0) {
        ctx.body = {
          code: 400,
          msg: '用户名已存在！',
        };
      ...
      doc.avatar = 'http://127.0.0.1:7001/public/head.jpeg'; // 登陆时默认要求有头像字段
