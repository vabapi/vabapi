'use strict';
const Collection = 'router'; // 指定本页接口操作的数据表，mongo中如果不存在会自动创建，无需操作数据库
...
class RouteradminController extends Controller {
  ...
  async doEdit() {
    ...
    if (request.id) {
      // 请求体包含id字段，说明是编辑
      /** 这里是编辑时不能重复字段的判定示例，多个字段复制多次，id列不需要判断
      ...
      const res = await vabapidb.findOneAndReplace(Collection, {
...
module.exports = RouteradminController;
